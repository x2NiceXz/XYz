...
Simple.Text = "XYz Spy | Best Script For You!"
...
function newRemote(type, name, args, remote, function_info, blocked, src)
	...
	schedule(function()
		...
		if blocked then
			logs[#logs].GenScript = "-- THIS REMOTE WAS PREVENTED FROM FIRING THE SERVER BY XYZSPY\n\n"
				.. logs[#logs].GenScript
		end
	end)
...
function genScript(remote, args)
	...
	gen = "-- Script generated by XYzSpy - credits to x2NiceXz\n\n" .. gen
	prevTables = {}
	return gen
end
	...
	for k, v in pairs(t) do -- iterates over table
		...
		if size > (_G.SimpleSpyMaxTableSize or 1000) then
			s = s
				.. "\n"
				.. string.rep(" ", l)
				.. "-- MAXIMUM TABLE SIZE REACHED, CHANGE '_G.XyzSpyMaxTableSize' TO ADJUST MAXIMUM SIZE "
			break
		end
...
if not _G.SimpleSpyExecuted then
	local succeeded, err = pcall(function()
		if not RunService:IsClient() then
			error("Xyz Spy cannot run on the server!")
		end
		if
			not hookfunction
	...
	if succeeded then
		...
		warn(
			"A fatal error has occured, Xyz Spy was unable to launch properly.\nPlease DM this error message to @x2NiceXz:\n\n"
				.. tostring(err)
		)
		SimpleSpy2:Destroy()
		hookfunction(remoteEvent.FireServer, originalEvent)
...
newButton("Function Info", function()
	...
	if selected then
		if selected.Function then
			codebox:setRaw(
				"-- Calling function info\n-- Generated by the Xyz Spy serializer\n\n" .. tostring(selected.Function)
			)
		end
		TextLabel.Text = "Done! Function info generated by the Xyz Spy Serializer."
	end
end)
...
newButton("Exclude (i)", function()
	return "Click to exclude this Remote.\nExcluding a remote makes Xyz Spy ignore it, but it will continue to be usable."
end, function()
	if selected then
...
newButton("Block (i)", function()
	return "Click to stop this remote from firing.\nBlocking a remote won't remove it from Xyz Spy logs, but it will not continue to fire the server."
end, function()
	if selected then
...
newButton("CallingScript", function()
	return string.format(
		"[%s] [UNSAFE] Uses 'getcallingscript' to get calling script for Decompile and GetScript. Much more reliable, but opens up Xyz Spy to detection and/or instability.",
